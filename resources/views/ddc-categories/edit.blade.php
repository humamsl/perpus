...
@include('partials.page-header', [
    'icon'  => 'fa-pen',
    'title' => 'Edit Kategori DDC',
    'desc'  => 'Perbarui data kategori: '.$item->code.' — '.$item->name,
    'actions' => [
        ['url' => route('ddc-categories.index'), 'label' => 'Kembali', 'class' => 'btn-secondary', 'icon' => 'fa-arrow-left'],
    ],
])

<form method="POST" action="{{ route('ddc-categories.update', $item) }}" class="card grid grid-cols-1 md:grid-cols-2 gap-4">@csrf @method('PUT')
    <div>
        <label class="text-sm font-semibold text-slate-700 dark:text-slate-200">Kode</label>
        <input name="code" required value="{{ $item->code }}" class="form-input mt-1 font-mono">
    </div>
    <div>
        <label class="text-sm font-semibold text-slate-700 dark:text-slate-200">Nama</label>
        <input name="name" required value="{{ $item->name }}" class="form-input mt-1">
    </div>
    <div>
        <label class="text-sm font-semibold text-slate-700 dark:text-slate-200">Induk</label>
    ...
    <div>
        ...
        <input type="number" name="order" value="{{ $item->order }}" class="form-input mt-1">
    </div>
    <div class="md:col-span-2">
        <label class="text-sm font-semibold text-slate-700 dark:text-slate-200">Deskripsi</label>
        <textarea name="description" class="form-textarea mt-1">{{ $item->description }}</textarea>
    </div>
