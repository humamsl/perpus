...
@include('partials.page-header', [
    ...
    'title' => 'Tambah Role',
    'desc'  => 'Buat role baru beserta hak aksesnya.',
])
...
<div class="card max-w-4xl">
    <form method="POST" action="{{ route('roles.store') }}">
        ...
        <div>
            ...
            <input name="name" required class="form-input mt-1 max-w-sm">
        </div>
        <div class="mt-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                @foreach($permissions as $p)
                    <label class="flex items-center gap-2 text-xs">
                        <input type="checkbox" name="permissions[]" value="{{ $p->name }}" class="rounded border-slate-300 text-primary-600 focus:ring-primary-500">
                        {{ $p->name }}
                    </label>
                @endforeach
            </div>
